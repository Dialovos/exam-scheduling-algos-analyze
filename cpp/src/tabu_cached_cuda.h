/*
 * Phase 3b — Tabu Search with batched CUDA move scoring.
 *
 * Same algorithm as solve_tabu_cached. Only difference: the candidate-scan
 * phase collects all (eid, pid, rid) triples into SoA arrays and batch-scores
 * them via CudaEvaluator::score_batch. On a CUDA-enabled build, this becomes
 * a single kernel launch per iteration; on a CPU-only build, CudaEvaluator
 * falls back to per-move Ecach.move_delta (same result, no speedup).
 *
 * CPU fallback is bit-exact equivalent to solve_tabu_cached on same seed —
 * that's the correctness validation. GPU speedup appears once libdelta_cuda.so
 * is linked (make cuda-build && make all HAVE_CUDA=1) and the kernel is
 * extended to cover the full fitness delta (currently conflict-only, with
 * soft-term correction on host).
 */
...
inline AlgoResult solve_tabu_cached_cuda(
    ...
    const Solution* init_sol = nullptr)
{
    ...
    CudaEvaluator Cuev(Ecach);  // owns device state if CUDA available, else CPU fallback
    ...
    if (verbose)
        std::cerr << "[TabuCachedCUDA] Init: feasible=" << ev.feasible()
                  << " hard=" << ev.hard() << " soft=" << ev.soft()
                  << " gpu=" << (Cuev.gpu_active ? "on" : "off (CPU fallback)")
                  << std::endl;
    ...
    // Scratch for batched scoring — reused across iters to avoid realloc churn
    std::vector<int32_t> mv_eid, mv_new_pid, mv_new_rid;
    std::vector<int> mv_ci;  // which candidate index each move came from (for dont-look bit)
    std::vector<double> batch_deltas;
    mv_eid.reserve(4096); mv_new_pid.reserve(4096);
    mv_new_rid.reserve(4096); mv_ci.reserve(4096);
    ...
    for (int it = 0; it < max_iterations; it++) {
        ...
        // Flatten every (eid, pid, rid) triple into SoA arrays, then make ONE
        // call into CudaEvaluator.score_batch. GPU: one kernel launch.
        // CPU: loop of score_delta_cpu_ref (the kernel's CPU twin — same
        // math as Ecach.move_delta, bit-exact by construction).
        mv_eid.clear(); mv_new_pid.clear();
        mv_new_rid.clear(); mv_ci.clear();
        for (int ci = 0; ci < n_cand; ci++) {
            ...
            for (int pid : cand_tgts[ci]) {
                ...
                for (int rid : valid_r[eid]) {
                    ...
                    mv_new_pid.push_back(pid);
                    mv_new_rid.push_back(rid);
                    mv_ci.push_back(ci);
                }
            }
        }
        ...
        Cuev.sync_state(sol);
        Cuev.score_batch(mv_eid, mv_new_pid, mv_new_rid, batch_deltas);

        int beid = -1, bpid = -1, brid = -1;
        double bdelta = 1e18;
        std::vector<uint8_t> cand_found_imp(n_cand, 0);

        for (size_t k = 0; k < mv_eid.size(); k++) {
            int ci = mv_ci[k];
            double d = batch_deltas[k];
            bool is_tabu = cand_is_tabu[ci];
            if (is_tabu && (current_fitness + d) >= best_fitness) continue;
            if (d < 0) cand_found_imp[ci] = 1;
            if (d < bdelta) {
                bdelta = d;
                beid = mv_eid[k];
                bpid = mv_new_pid[k];
        ...
        // Don't-look bits: mark candidates with no improving move found
        for (int ci = 0; ci < n_cand; ci++) {
            if (!cand_skip[ci] && !cand_found_imp[ci] && !cand_is_bad[ci])
                dont_look[candidates[ci]] = 1;
        }

        // ── Swap moves ── (identical to tabu_cached.h)
        ...
        // ── Kempe chain ── (identical to tabu_cached.h)
        ...
        if (current_fitness < best_fitness - 0.5) {
            ...
            if (!dominated && af < best_fitness) {
                ...
                if (verbose && (it < 10 || it % 200 == 0))
                    std::cerr << "[TabuCachedCUDA] Iter " << it << ": best hard=" << check.hard()
    ...
    if (verbose)
        std::cerr << "[TabuCachedCUDA] " << iters_done << " iters, " << rt << "s"
                  << "  feasible=" << final_ev.feasible()
                  << " hard=" << final_ev.hard()
                  << " soft=" << final_ev.soft()
                  << "  gpu=" << (Cuev.gpu_active ? "on" : "off") << std::endl;

    return {std::move(best_sol), final_ev, rt, iters_done, "Tabu (Cached+CUDA)"};
}
